defmodule HierbautberlinWeb.UserSessionControllerTest do
  ...
  describe "POST /users/log_in" do
    ...
    test "logs the user in with remember me", %{conn: conn, user: user} do
      ...
      assert conn.resp_cookies["_hierbautberlin_web_user_remember_me"]
      assert redirected_to(conn) =~ "/"
    end
    ...
    test "emits error message with invalid credentials", %{conn: conn, user: user} do
      ...
      response = html_response(conn, 200)
      assert response =~ "<h1>Anmelden</h1>"
      assert response =~ "Ungültige Email-Adresse oder Passwort."
    end
  end
