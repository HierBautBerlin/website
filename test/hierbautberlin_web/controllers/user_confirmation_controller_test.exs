defmodule HierbautberlinWeb.UserConfirmationControllerTest do
  ...
  describe "POST /users/confirm" do
    ...
    test "sends a new confirmation token", %{conn: conn, user: user} do
      ...
      assert get_flash(conn, :info) =~ "Wenn deine Email-Adresse in unserem System ist"
  ...
  describe "GET /users/confirm/:token" do
    test "confirms the given token once", %{conn: conn, user: user} do
      ...
      assert redirected_to(conn) == "/"
      assert get_flash(conn, :info) =~ "Konto erfolgreich bestätigt."
      ...
      conn = get(conn, Routes.user_confirmation_path(conn, :confirm, token))
      assert redirected_to(conn) == "/"
      assert get_flash(conn, :error) =~ "Konto Bestätigungslink ist leider nicht mehr gültig."
